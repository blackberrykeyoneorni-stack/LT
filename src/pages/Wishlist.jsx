...
export default function Wishlist() {
    ...
    useEffect(() => {
        ...
        // 1. Live-Listener für die Wishlist
        const q = query(
        ...
        const loadBudget = async () => {
            try {
                ...
                if (snap.exists()) {
                    setBudgetState({
                        limit: snap.data().monthlyLimit || 0,
                        spent: snap.data().currentSpent || 0
                    });
                }
            } catch(e) {
                console.error("Budget konnte nicht geladen werden", e);
            }
        };
        loadBudget();
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Diesen Wunsch löschen?")) {
    ...
    return (
        <Container maxWidth="sm" sx={{ pb: 10 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, mt: 2 }}>
                ...
                <Chip 
                    icon={<SavingsIcon />} 
                    label={`${available.toFixed(2)} € verfügbar`} 
                        ...
                        return (
                            <Paper 
                                ...
                                sx={{ 
                                    ...
                                    borderLeft: isAffordable ? '4px solid #4caf50' : '4px solid transparent',
                                ...
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    <Box>
                                        ...
                                        <Stack direction="row" spacing={1} sx={{ mt: 0.5, alignItems: 'center' }}>
                                            <Chip 
                                                label={`${wish.price.toFixed(2)} €`} 
                                                size="small" 
                                                color={isAffordable ? "success" : "default"} 
                                                variant={isAffordable ? "filled" : "outlined"}
                                            />
                                            {isAffordable && (
                                    ...
                                    <IconButton size="small" onClick={() => handleDelete(wish.id)} sx={{ color: 'text.secondary' }}>
                                        <DeleteIcon />
                                    </IconButton>
                                </Box>
                            </Paper>
            ...
            <Dialog 
                ...
                PaperProps={{ sx: DESIGN_TOKENS.glassCard }}
                fullWidth
            >
                <DialogTitle>Neuer Wunsch</DialogTitle>
                <DialogContent>
                    <Stack spacing={2} sx={{ mt: 1 }}>
                        <TextField 
                            label="Was möchtest du?" 
                            fullWidth 
                            autoFocus
                            value={newItem.name}
                        ...
                        >
                            <option value="high">Hoch (Muss ich haben)</option>
                            <option value="medium">Mittel</option>
                            <option value="low">Niedrig (Irgendwann)</option>
                        </TextField>
                    </Stack>
                </DialogContent>
                <DialogActions>
