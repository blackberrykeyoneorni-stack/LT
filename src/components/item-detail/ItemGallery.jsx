...
export default function ItemGallery({ item, isEditing, onToggleEdit, onSave, onWriteNFC, onAddImages, onRemoveImage, displayImages }) {
    const navigate = useNavigate();
    const { prepareSystemDialog } = useSecurity(); 
    const [overlayOpen, setOverlayOpen] = useState(false);
    const [currentFullscreenIndex, setCurrentFullscreenIndex] = useState(0);

    const images = displayImages !== undefined 
        ? displayImages 
    ...
    return (
        <Box sx={{ 
            ...
            {images.length > 0 ? (
                        ...
                        <SwiperSlide key={index}>
                            <Box sx={{ position: 'relative', width: '100%', height: '100%' }}>
                                <img 
                                    ...
                                    onClick={() => { if (!isEditing) { setCurrentFullscreenIndex(index); setOverlayOpen(true); } }} 
                                />
                                {isEditing && (
                                    <IconButton
                ...
                <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', color: PALETTE.primary.main }}>
                    <PhotoCamera sx={{ fontSize: 60, mb: 2, opacity: 0.5 }} />
                    <Typography sx={{ opacity: 0.5, textTransform: 'uppercase', letterSpacing: 2 }}>Kein Bild</Typography>
                </Box>
            )}
